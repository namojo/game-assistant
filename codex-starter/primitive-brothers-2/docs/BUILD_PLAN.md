...
**총 태스크 41개** (P0:4 / P1:10 / P2:**9** / P3:4 / P4:5 / P5:4 / P6:5). 각 태스크 = Codex 1세션(S: 30분 미만 / M: 60분 미만). P2가 8→9로 늘어난 것은 리더 결정(I-7)에 따라 `sim replay` 재현 태스크 **T2.9**를 신설했기 때문이다.
...
3. **확률 필드 규칙 검사**: 이름이 `*_ppm`으로 끝나는 전 필드는 `Number.isInteger(v) && 0 <= v <= 1_000_000`이어야 한다(스키마 `type: integer` + `minimum`/`maximum`으로 1차, 로더가 2차로 단언). 시행 단위 확률 집합(`drop_rates_ppm.<system>`)은 **정수 합이 정확히 1,000,000**이어야 한다. 위반 시 **기동 실패**. 같은 규칙을 `tools/check-deps.ts`가 소스 쪽에서도 검사한다 — 코드·테스트에 부동소수 확률 리터럴(`0.005` 등)을 쓰면 exit 1.
4. **공통 메타 검증**: 모든 데이터 파일의 `schema_version`·`generated_at`·`checksum` 3필드 존재를 확인하고, `checksum`을 본문에서 재계산해 대조한다(3.1). `data/_manifest.json`의 `checksum`(파일 내용 SHA-256, 개행 정규화 후)과도 교차 대조한다. 불일치 시 **기동 실패**.
5. 참조 무결성 검사(스키마로는 못 잡는 것): `brothers.skin_ids ⊆ skins.id`, `monsters.codex_id ∈ collection.id`, `equipment.merge_from ∈ equipment.id`, `stages.monster_pool ⊆ monsters.id`, `*.asset_id ∈ assets/_index.json`.
6. 파생 테이블 생성(1회): 스테이지 HP/골드 테이블, 강화 비용 테이블, 노드 비용 누적 테이블 (D-3).
7. 결과를 `Tables` 불변 객체로 얼려(`Object.freeze` 재귀) 코어에 주입한다.
...
| `npm run sim:replay -- --run <run.json>` | 기록된 실행을 시드 + 테이블 checksum으로 재현 (규격 `sim replay` 별칭, T2.9). 재현 실패·checksum 불일치는 **exit 2** |
...
| `npm run check:deps` | 의존 방향·금지 식별자 검사 (1.1) + **확률 리터럴 검사**(소스·테스트에 부동소수 확률 금지, `*_ppm` 정수 0~1,000,000 규칙 — 1.6) |
...
- **확률·비율은 정수 ppm(백만분율)이다.** 필드명은 `*_ppm` 접미, 값은 **0~1,000,000 정수**, 스키마는 `"type": "integer"` + `"minimum": 0` + `"maximum": 1000000`. 로드 시 `p = ppm / 1_000_000`으로 변환해 코어에 넘기고, 파일·코드·테스트에 부동소수 확률을 쓰지 않는다(GDD 8.0, 전략 청사진 4.3.1, 기획서 부록 E). 시행 단위 확률 집합은 **정수 합 = 1,000,000** 단언. 시간은 초, 배수는 `number`.
- **테이블 JSON 공통 메타 3필드(전 파일 필수)**: `schema_version`(문자열, 예 `"1.0"`), `generated_at`(ISO8601 UTC), `checksum`(`"sha256:<hex>"` — 본문 직렬화·개행 정규화 후). 배열 테이블은 `{schema_version, table, generated_at, source, checksum, rows}` 봉투로 싸고 `rows`가 기존 배열이다(`balance.json`은 `rows` 대신 `row` 단일 객체). 로더는 3필드 누락 시 **기동 실패**하고 `checksum`을 재계산해 대조한다. 근거: 확률 검증 리포트가 "어느 테이블로 돌린 결과인가"를 가리키는 앵커이며 `sim replay`(T2.9)의 재현 키다(전략 청사진 팀원 전달 메모 (2)-①(e)) → 8.3 A-D15.
...
| `equipment` | id, name_ko, slot, tier, mult, merge_count, drop_stage_min, drop_weight_ppm, asset_id | `tier` 0~40, **`mult == equip_tier_mult^tier` (로더 파생 검사, 오차 1e-12)**, `merge_count == 4`, **`drop_weight_ppm` 정수 0~1,000,000**(0.35 → `350000`), `merge_from` null 또는 tier-1 항목 |
...
```json
{
  ...
  "drop_rates_ppm": { "bone_grind": { "legend": 5000, "hero": 30000, "rare": 150000, "common": 815000 } },
  "pity_hero": 20, "pity_legend": 40
}
```

스키마 강제 제약: `drop_rates_ppm.bone_grind`의 4개 값은 **전부 0~1,000,000 정수**이고 **합이 정확히 1,000,000**이다(로더 검사 — 부동소수 허용 오차가 없어진다), `node_cost_slope > 0`(등차 보장), `equip_recovery_mode` enum `["instant","drop_wait"]`, `version` integer ≥ 1. 공통 메타 3필드(`schema_version`·`generated_at`·`checksum`)는 3.1대로 이 파일에도 붙는다.
...
- 규칙: 3.2의 MVP 행 수를 그대로 채운다. 3.3의 굵은 제약을 스키마에 반영한다. `balance.json`은 3.4를 **한 글자도 바꾸지 않고** 그대로 쓴다. `totems.json`은 계열 1행 + `order:0` (589행 금지). **확률 필드는 전부 `*_ppm` 정수(0~1,000,000)로 쓰고 스키마에 `type: integer` + `minimum: 0` + `maximum: 1000000`을 박는다**(3.1). **전 데이터 파일에 공통 메타 3필드(`schema_version`·`generated_at`·`checksum`)를 붙이고 로더가 checksum을 재계산해 대조한다**(3.1, 1.6-4).
- 수용 기준:
  1. `npm test -w packages/data` → **22 passed, 0 failed**
  2. 테스트에 다음 실패 케이스가 포함되고 각각 기동 실패를 단언: ① `skins[0].stat_bonus.value = 0.06` → 스키마 오류 ② `totems[0].order = 5` → 스키마 오류 ③ `equipment[3].mult` 임의 변경 → 파생 검사 오류 ④ `monsters[0].codex_id = "cdx_none"` → 참조 무결성 오류 ⑤ `_manifest.json` 체크섬 1자 변경 → 체크섬 오류 ⑥ `drop_rates_ppm.bone_grind`의 정수 합 ≠ 1,000,000 → 합계 오류 ⑦ `stages` 구간 중첩 → 구간 오류 ⑧ **경계**: `brothers`가 빈 배열 → minItems 오류 ⑨ **확률 단위**: `drop_rates_ppm.legend = 0.005`(소수) → `type: integer` 오류 / `= 1000001`(범위 초과) → `maximum` 오류 / `equipment[0].drop_weight_ppm = 0.35` → 정수 오류 ⑩ **공통 메타**: `balance.json`의 `checksum`을 1자 변경 → 재계산 불일치로 기동 실패, `schema_version` 필드 삭제 → 필수 필드 오류
  3. `npx tsx packages/data/src/load.ts --print-summary` → `tables loaded: 12, rows: 60, schema_version 1.0, checksum OK (12/12 recomputed), integrity OK, ppm OK` 형태 출력, 종료 코드 0
  5. `npm run check:deps` → 종료 코드 0. `packages/core/src/equip.ts`에 `const p = 0.005;`를 임시로 넣고 다시 실행 → **종료 코드 1**, `float probability literal` 위반 출력 (되돌린다)
  6. `grep -rn "drop_rates\b" data/ packages/ --include=*.json --include=*.ts` → **0줄**(구 키 `drop_rates`가 남아 있지 않음. 정본 키는 `drop_rates_ppm`)
...
- 산출: `tools/validate-assets.ts` + 테스트, `assets/_index.json`(약 34항목), `assets/source/.gitkeep`, `assets/gen/.gitkeep`, 루트 `package.json`의 `prebuild`/`pretest` 훅, **`simple-git-hooks` 기반 pre-commit 훅 설정**(`package.json`의 `simple-git-hooks.pre-commit` = `npm run validate:assets`)
...
- 수용 기준:
  ...
  5. **pre-commit 게이트(리더 결정 ⑥)**: `npx simple-git-hooks` 실행 후 `git config core.hooksPath` → `.git/hooks` 경로 출력, `cat .git/hooks/pre-commit` → `npm run validate:assets` 포함. 위반 에셋(`reviewed_by:""`)을 스테이징하고 `git commit -m "t"` → **종료 코드 1**, 커밋이 생성되지 않음(`git log -1 --format=%s`가 직전 커밋 그대로) 후 되돌린다
  6. `npm test -w tools -- hooks` → **2 passed**: ① `package.json`에 `simple-git-hooks.pre-commit` 키가 존재하고 값이 `validate:assets`를 포함 ② `postinstall`에 `simple-git-hooks` 등록이 있어 `npm ci` 후 훅이 자동 설치됨
- 크기: S
- 참고: `prebuild`/`pretest`는 npm이 자동 실행한다. CI에도 독립 잡으로 넣어서, 누군가 `--ignore-scripts`로 우회해도 머지가 막히게 한다. **로컬 우회(`npm ci --ignore-scripts`)를 막는 것이 5·6번 기준의 목적이다**(리더 결정 ⑥) — 훅 관리자는 `simple-git-hooks`로 **지정한다**(husky는 쓰지 않는다. 의존성이 더 작고 `package.json` 한 블록으로 끝나기 때문). `git commit --no-verify`로 뚫리는 것은 남지만, 그 경우에도 CI 독립 잡이 머지를 막는다 — 로컬 게이트는 "실수 방지", CI 잡은 "우회 방지"로 역할이 나뉜다.
...
- 규칙:
  - 드랍 등급은 `floor(s / equip_tier_span)` 중심의 결정적 분포. 드랍 판정은 `rng.fork("drop")` 스트림만 사용. **드랍 가중치는 `equipment[].drop_weight_ppm`(정수 0~1,000,000)을 읽어 `w = ppm / 1_000_000`으로 변환해 쓴다** — 코드에 소수 가중치를 쓰지 않는다(3.1).
...
---
### Phase 2 — 헤드리스 시뮬레이터와 밸런스 회귀 (9태스크)
...
- 목표: 표기 확률을 `balance.drop_rates_ppm`(정수 ppm)에서 읽어 `p = ppm / 1_000_000`으로 변환한 뒤 `N ≥ 900(1−p)/p` 규칙으로 시행수를 자동 산출하고 검증한다.
- 의존: T2.4
- 입력: GDD 7.3, 밸런스 모델 8.4, 모호 지점 9
- 산출: `packages/sim/src/prob.ts`, `npm run sim:prob` + 테스트
- 규칙:
  - 시행수: `N_i = max(100_000, ceil(900 × (1 − p_i) / p_i))`를 100,000 단위로 올림. → 전설(5,000 ppm = 0.005) **N = 1,800,000**, 영웅(30,000 ppm) N = 100,000, 희귀(150,000 ppm) N = 100,000, 일반(815,000 ppm) N = 100,000.
  - **단위**: 입력·출력·리포트의 표기값은 **정수 ppm**이다. 소수 `p`는 3σ 계산의 중간값으로만 쓰고 파일에 쓰지 않는다. 4개 값의 **정수 합이 1,000,000**임을 같은 하네스가 단언한다.
  - 판정: 관측 빈도가 `p ± 3√(p(1−p)/N)` 안.
  - **표기값 하드코딩 금지**: 테스트는 `balance.json`의 `drop_rates_ppm`을 읽는다. `grep`으로 검증한다(부동소수 확률 리터럴도, ppm 정수 리터럴도 금지).
...
- 수용 기준:
  1. `npm run sim:prob` → 4행 표 출력(`grade, declared_ppm, p, N, observed_ppm, 3σ band, verdict`)에서 **4/4 PASS**, 종료 코드 0. 임계 위반 시 **종료 코드 2**(6.1 exit code 규약)
  2. 출력의 `declared_ppm` 열이 `5000 / 30000 / 150000 / 815000`(합 1,000,000), `N` 열이 `1800000 / 100000 / 100000 / 100000`과 일치
  ...
  4. `grep -rn "0\.005\|0\.815\|\b0\.03\b\|\b0\.15\b\|\b5000\b\|\b30000\b\|\b150000\b\|\b815000\b" packages/sim/src/prob.ts packages/sim/src/prob.test.ts` → **0줄**(부동소수·ppm 정수 양쪽 다 하드코딩 금지)
  5. 전설 1.8e6 시행 실행 시간이 **60초 이내**. **60초를 초과하면(리더 결정 ⑦) `PROB_N_PROFILE=ci` 환경변수로 전설 N = 200,000 폴백을 허용한다** — 폴백값은 산식 최소치 **179,100 이상**이어야 하고(하네스가 `N >= ceil(900(1−p)/p)`를 단언), 리포트 머리글에 `profile: ci (N=200000, formula min 179100)`을 출력한다. 기본(증거 생산용) 프로파일은 그대로 1,800,000이며, 폴백은 **CI 프로파일에서만** 쓴다
- 크기: M
...
#### T2.9 `sim replay` — 시드 + 테이블 checksum 재현 (규제 증거)
- 목표: 과거 실행 기록(`run.json`)만으로 **같은 결과를 다시 만들어**, 확률 검증 리포트가 법적 방어 증거로 성립하게 한다.
- 의존: T2.3, T2.4
- 입력: 전략 청사진 4.3.1(증거 3종 중 ① 시드 고정 재현 로그)·팀원 전달 메모 (2)-②, 본 문서 3.1(공통 메타)·6.1(서브커맨드·exit code)
- 산출: `packages/sim/src/replay.ts`, `npm run sim:replay` 스크립트 + 테스트
- 규칙:
  - `npm run sim:replay -- --run <run.json>` — `run.json`(= `reports/{runId}/summary.json`)에 기록된 `seed`·`tables_checksum`·`schema_version`·CLI 인자 전량을 읽어 같은 실행을 재구성한다. 사람이 인자를 다시 입력하지 않는다.
  - T2.1이 `summary.json`에 `{"seed", "args", "tables_checksum", "schema_version", "generated_at"}`를 기록하는 것이 선행 조건이다(3.1 공통 메타와 같은 값).
  - **checksum 불일치는 폴백 없이 exit 2**: 테이블이 바뀐 상태의 재현은 증거가 아니다. stderr에 `tables changed: <table> expected <sha> got <sha>`를 출력한다.
  - 재현 판정은 **T2.4와 같은 비교기**(`runId` 제외 전 파일 SHA-256 + 최종 상태 해시)를 import해서 쓴다. 비교 로직을 두 벌 만들지 않는다.
  - stdout은 기계 판독 JSON 1개 객체(`{"replay": "<runId>", "match": true|false, "diff_files": [...]}`), 사람용 요약은 stderr(6.1 출력 규약).
- 수용 기준:
  1. `npm run sim -- --days 7 --bot balanced --seed 42` 실행 후 `npm run sim:replay -- --run reports/balanced-f2p-42-7d/summary.json` → stdout `{"replay":"balanced-f2p-42-7d","match":true,"diff_files":[]}`, stderr `replay OK: 6/6 files identical, state hash match`, 종료 코드 **0**
  2. `data/balance.json`의 `hp_growth`를 1.060으로 바꾸고 `npx tsx tools/gen-manifest.ts` 후 같은 명령 → 종료 코드 **2**, stderr에 `tables changed: balance expected sha256:… got sha256:…` (변경은 되돌린다)
  3. `summary.json`의 `seed`를 `43`으로 바꾸고 같은 명령 → 종료 코드 **2**, stdout `"match":false`, `diff_files`에 최초로 갈라진 파일명이 들어감
  4. `npm test -w packages/sim -- replay` → **5 passed**. 포함: ① 비교기가 T2.4와 동일 모듈임을 단언(`determinism.ts`의 `compareRun()`을 import) ② 공통 메타 3필드가 없는 `run.json` → 종료 코드 **1**(임계 위반이 아니라 실행 오류) ③ **경계**: `--run` 누락 → 사용법 출력 후 종료 코드 1 ④ **경계**: 30일 실행 기록도 재현 성공
  5. 확률 검증 산출물도 재현 가능: `npm run sim:prob -- --out reports/prob` 후 `npm run sim:replay -- --run reports/prob/summary.json` → 종료 코드 0, 관측 빈도(ppm)가 **완전히 동일한 값**으로 재산출
- 크기: S
- 참고: 이 태스크는 회귀 게이트가 아니라 **증거 생산 장치**다. 결정성 테스트(T2.4)는 "같은 인자로 2회 돌리면 같은가"를 보고, 이 태스크는 "**기록만으로 과거를 되살릴 수 있는가**"를 본다 — 규제 대응에서 필요한 것은 후자다. exit 2를 쓰는 이유는 6.1 규약에서 임계 위반 = 배포 차단이고, 재현 불가는 같은 등급의 사건이기 때문이다.
...
- 규칙:
  ...
  - 표기값은 **`balance.drop_rates_ppm`(정수 ppm)에서 읽어** `p = ppm / 1_000_000` 변환 후 퍼센트로 **렌더링**한다. UI에 숫자를 타이핑하면 표기-구현 불일치가 규제 리스크가 된다.
- 수용 기준:
  ...
  5. `grep -rn "0\.5%\|0\.005\|81\.5\|\b5000\b\|\b815000\b" apps/web/src/ui/` → **0줄**(표기 하드코딩 금지 — 퍼센트·소수·ppm 정수 전부)
- 크기: M
...
| 13.3 | 성공 판정 4개 | 1→T2.5 / 2→**T2.4, T2.9**(재현) / 3→T2.7 / 4→T1.1 수용 기준 5 |
| 7.3 | 확률 검증 **증거 3종**(전략 청사진 4.3.1) | ① 시드 고정 재현 로그 → **T2.9** / ② 대량 시행 분포 검증 리포트 → **T2.6** / ③ 버전별 확률 테이블 diff 기록 → **T2.7** |
...
npm run sim:replay -- --run reports/balanced-f2p-42-7d/summary.json   # 기록 재현 (T2.9)
...
| `--run` | `run.json` 경로 (`sim:replay` 전용) | 필수 |
| `--format` | `json` \| `md` \| `json,md` | `json` |
| `--thresholds` | 임계 설정 파일 (`sim:diff` 전용 — 코드 상수 금지) | `config/thresholds.json` |

#### 6.1.1 규격 서브커맨드 ↔ npm 스크립트 별칭 (정본 대조표)

전략 청사진 팀원 전달 메모 (2)-②가 규정한 서브커맨드 체계에 맞춘다. **npm 스크립트 이름은 그대로 유지하고**(워크스페이스 관례 + CI 잡과 1:1), 아래 표가 규격 ↔ 별칭의 정본이다. 전작 시뮬레이터가 합류할 때는 이 표의 왼쪽 열을 공통 인터페이스로 삼는다 → 8.3 A-D15.

| 규격 서브커맨드 | 본 저장소 별칭 | 대응 태스크 | 비고 |
|---|---|---|---|
| `sim run` | `npm run sim -- …` | T2.1 | 옵션명 동일(`--days` `--seed` `--profile` `--out` `--format`). 규격의 `--tables <dir>`는 `data/`가 고정이라 생략하고, `--runs <N>`은 `--days`와 시드 목록으로 대체 |
| `sim diff` | `npm run sim:diff -- --base … --head … --thresholds …` | T2.7 | 임계는 설정 파일로 분리(코드 상수 금지) |
| `sim verify-odds` | `npm run sim:prob` | T2.6 | 표기표는 `balance.drop_rates_ppm`(정수 ppm)에서 읽는다 |
| `sim replay` | `npm run sim:replay -- --run <run.json>` | **T2.9(신설)** | 시드 + 테이블 checksum으로 동일 결과 재현 |
| (규격 외 · CI 전용) | `npm run sim:regress` | T2.5 | `sim run` + 회귀 단언 13개 묶음 게이트 |
| (규격 외 · CI 전용) | `npm run sim:determinism` | T2.4 | 같은 인자 2회 바이트 동일 |
| (규격 외 · 리포트) | `npm run sim:long` | T2.8 | 60·90일 장기 프로파일, 게이트 아님 |

#### 6.1.2 exit code 규약 (전 서브커맨드 공통)

| 코드 | 의미 | CI 동작 |
|---|---|---|
| `0` | 정상 — 판정 PASS | 통과 |
| `1` | **실행 오류** — 인자 오류, 파일 없음, 스키마 위반, 공통 메타 누락 | 실패(빌드 오류로 취급) |
| `2` | **임계 위반** — 회귀 밴드 초과, 확률 3σ 이탈, 천장 단언 실패, 재현 불일치 | 실패 + **배포 차단**(deploy 잡의 선행 게이트) |

1과 2를 나누는 이유는 CI가 **"우리가 잘못 실행했다"와 "제품이 기준을 위반했다"를 다르게 다뤄야** 하기 때문이다. 2는 사람 판단 없이 배포를 막는다.

#### 6.1.3 출력 규약

- **stdout = 기계 판독 JSON 1개 객체.** 사람용 문장을 섞지 않는다(에이전트가 파싱한다).
- **stderr = 사람용 요약** 몇 줄.
- `--format md`를 주면 마크다운 리포트를 `--out` 경로에 추가로 쓴다. `--format json,md`로 둘 다 낼 수 있다.
...
| T5 | 확률 표기값(정수 ppm)을 `balance.drop_rates_ppm`에서 읽어 검증 + 정수 합 1,000,000 단언 | T2.6 | `npm run sim:prob` | CI 필수 |
...
| +4 | 기록(`run.json`)만으로 재현 — 시드 + 테이블 checksum 일치 | T2.9 | `npm run sim:replay -- --run <run.json>` | CI 필수(확률 리포트 생산 시) |

### 6.5 확률 검증 시행수 (모호 지점 9의 결정)

`N_i = max(100_000, ceil(900 × (1 − p_i) / p_i))`를 100,000 단위로 올림. 입력은 `balance.drop_rates_ppm`의 **정수 ppm**이고 `p_i = ppm_i / 1_000_000`이다.

| 등급 | 표기값(ppm) | 표기 p | 산식값 | 채택 N | 3σ 허용 구간 |
|---|---|---|---|---|---|
| 전설 | **5,000** | 0.005 | 179,100 | **1,800,000** (산식값의 약 10배 여유) | p ± 3√(p(1−p)/N) = 0.005 ± 0.000158 |
| 영웅 | **30,000** | 0.03 | 29,100 | 100,000 | 0.03 ± 0.001618 |
| 희귀 | **150,000** | 0.15 | 5,100 | 100,000 | 0.15 ± 0.003387 |
| 일반 | **815,000** | 0.815 | 204 | 100,000 | 0.815 ± 0.003676 |

(합 1,000,000 — 정수 합 단언, 3.1)

전 확률에 N=100,000을 일괄 적용하면 전설의 3σ 상대오차가 13.4%로 ±10% 기준을 못 넘는다. 그래서 산식을 코드에 넣고 표기표에서 자동 산출한다. **전설의 채택 N(1,800,000)은 산식값(179,100)보다 한 자릿수 크다 — 규제 증거용 여유분이며 리더 결정 ⑦로 유지가 확정됐다. 다만 T2.6 수용 기준 5(60초 이내)를 못 지키는 경우에 한해 `PROB_N_PROFILE=ci`에서 N = 200,000 폴백을 허용한다(산식 최소 179,100 이상 단언). 증거 생산용 기본 프로파일은 1,800,000 그대로다.**
