...
npm run sim -- --days 7 --bot balanced --seed 42    # 시뮬레이터 1회        [= sim run]
npm run sim:regress                                 # 목표 벽 표 10 체크포인트 ±10% + 첫 벽 + 격차 + D1 프레스티지
npm run sim:prob                                    # 확률 표기 검증 + 천장 단언  [= sim verify-odds]
npm run sim:determinism                             # 동일 시드 2회 바이트 동일
npm run sim:diff -- --base bal_v1 --head bal_v2     # 곡선 diff 리포트        [= sim diff]
npm run sim:replay -- --run <run.json>              # 기록 재현 (시드 + 테이블 checksum)  [= sim replay]
...
**서브커맨드 별칭**: 위 대괄호가 규격 서브커맨드(`sim run` / `sim diff` / `sim verify-odds` / `sim replay`)와의 대응이다. 정본 대조표는 `docs/BUILD_PLAN.md` 6.1.1에 있다. **npm 스크립트 이름을 바꾸지 않는다.**

**exit code 규약** (전 시뮬레이터 명령 공통 — `docs/BUILD_PLAN.md` 6.1.2)

| 코드 | 의미 | 에이전트 행동 |
|---|---|---|
| `0` | 정상(PASS) | 다음 단계로 |
| `1` | 실행 오류(인자·파일·스키마·공통 메타 누락) | 스스로 고친다 |
| `2` | **임계 위반**(회귀 밴드·확률 3σ·천장·재현 불일치) | **`data/balance.json`을 건드리지 않고 보고한다.** CI에서는 배포가 차단된다 |

**출력 규약**: stdout은 기계 판독 JSON 1개 객체, 사람용 요약은 stderr, 마크다운 리포트는 `--format md`. **stdout에 사람용 문장을 섞지 않는다.**
...
7. 수치를 코드에 하드코딩하지 않는다. 전부 `data/*.json`에서 읽는다. 확률 표기값을 테스트나 UI에 타이핑하면 검증 의미가 사라진다 — 반드시 `balance.drop_rates_ppm`에서 읽는다.
7-1. **확률·비율은 정수 ppm(백만분율)이다.** 필드명은 `*_ppm`, 값은 **0~1,000,000 정수**, 로드 시 `p = ppm / 1_000_000`으로 변환해 쓴다. 시행 단위 확률 집합은 **정수 합 = 1,000,000**. 코드·테이블·테스트에 **부동소수 확률 리터럴(`0.005` 등)을 쓰지 않는다** — `npm run check:deps`가 exit 1로 막는다. 소수 `p`는 계산 중간값으로만 존재한다.
7-2. **모든 데이터 파일에 공통 메타 3필드**(`schema_version`·`generated_at`·`checksum`)가 있다. 필드를 지우거나 `checksum`을 손으로 고치지 않는다. 테이블을 바꿨으면 `npx tsx tools/gen-manifest.ts`로 재생성한다. 이 셋은 확률 검증 리포트와 `sim:replay`의 재현 앵커다.
...
13. `assets/_index.json`에서 **`origin:"ai"`인 항목은 `source_id`와 `reviewed_by`가 필수**다. 비어 있으면 `npm run build`와 `npm test`가 실패한다.
13-1. **`--ignore-scripts`로 게이트를 우회하지 않는다.** `npm ci --ignore-scripts`, `npm install --ignore-scripts`, `git commit --no-verify`, `prebuild`/`pretest`/`pre-commit` 훅 삭제·주석 처리는 전부 금지다. 설치는 항상 `npm ci`로 하고, pre-commit 훅(`simple-git-hooks`)이 설치돼 있는지 `cat .git/hooks/pre-commit`으로 확인한다. 이 게이트는 "핵심 캐릭터 원화는 사람이 그린다"는 거버넌스를 **코드로 강제하는 유일한 장치**이고, 그 약속은 고객 문서에 이미 적혀 있다 — 로컬에서 뚫리는 게이트는 약속의 근거가 되지 못한다.
